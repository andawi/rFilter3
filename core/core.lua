  ...
  rFilter3 = LibStub("AceAddon-3.0"):NewAddon("rFilter3")
  
  -----------------------------
  -- FUNCTIONS
  -----------------------------
local f = CreateFrame("Frame", "CursorAnchorFrame", UIParent)
	...
	f:SetScript("OnUpdate", function(self, elapsed)
		rFilter3:OnUpdate(self, elapsed)
	end)
  
  
function rFilter3:OnUpdate(frame, elapsed)

	--Anchor to Cursor
	local x, y = rFilter3:GetCursorPosition(CursorAnchorFrame)
	CursorAnchorFrame:ClearAllPoints()
	CursorAnchorFrame:SetPoint( "BOTTOMLEFT", UIParent, "BOTTOMLEFT", x, y )
		
end
  
  
-- Cursor Position and Scale depending on passed frame
function rFilter3:GetCursorPosition(frame)
...
function glowOffdelay(frame, time)
	local a = CreateFrame("Frame")		-- optimieren
    local ag = a:CreateAnimationGroup()
    local anim = ag:CreateAnimation()
    anim:SetDuration(time or 3)
...
function spellOnCD(frame, time)
	
	frame.iconframe.icon:SetAlpha(frame.alpha.cooldown.icon)
	frame.iconframe:SetAlpha(frame.alpha.cooldown.frame)
    ...
    ag:SetScript("OnFinished", function(self, event, ...)
		...
		frame.iconframe:SetAlpha(frame.alpha.no_cooldown.frame)
		frame.iconframe.icon:SetAlpha(frame.alpha.no_cooldown.icon)
		
		if frame.sound then PlaySoundFile(frame.sound, "Master") end
		--glow	  
		if frame.glow then 
			ActionButton_ShowOverlayGlow(frame.iconframe)
			glowOffdelay(frame.iconframe, frame.glowtrail)
		end
		frame.onCD = false
    end)
    ag:Play()
  ...
  local applySizeChange = function(i)
    ...
    if i.border then i.border:SetSize(w*1.18,w*1.18) end
  ...
  local createIcon = function(f,index,type)
	...
	local CursorAnchorFrame = _G["CursorAnchorFrame"]
	local i = CreateFrame("FRAME",makeFrameName(f,type), CursorAnchorFrame)
	
	if f.setToCursor == true then
		i:SetPoint("CENTER", f.pos.x, f.pos.y)
	else
   ...
   ba:SetAllPoints(i)
   ba:SetTexture(0.15,0.15,0.15,0.25)
	...
	local cd = CreateFrame("Cooldown", nil, i, "CooldownFrameTemplate")
	cd:SetAllPoints(i)
	i.cd = cd
  ...
  local checkAura = function(f,filter,spellid)
    ...
    if f.name and f.rank then
      ...
      if name and (not f.caster or (f.caster and caster == f.caster)) and (not f.match_spellid or (f.match_spellid and spID == tmp_spellid)) then
        if caster == "player" and cfg.highlightPlayerSpells and f.iconframe.border then
          f.iconframe.border:SetVertexColor(0.2,0.6,0.8,1)
        elseif cfg.highlightPlayerSpells and f.iconframe.border then
        ...
        f.iconframe.value:SetText("")
        f.iconframe.time:SetTextColor(1, 0.8, 0)
        if cfg.highlightPlayerSpells and f.border then
          f.iconframe.border:SetVertexColor(0.37,0.3,0.3,1)
        end
        if f.desaturate then
  ...
  -----------------------------
  --[[
  improve: check only on certain events
  ]]
  
  local checkCooldown = function(f)
    
	if f.onCD == true then return end
	...
	if f.name and f.spellid then
      ...
      if start and duration then
		...
		--print("name: "..f.name.."     value:"..value.."     duration:"..duration)
		if value > 0 and duration > 1.51 then
				spellOnCD(f, duration)
				f.onCD = true
        end
      end
    end
